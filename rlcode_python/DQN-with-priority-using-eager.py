    ...
    for i in tqdm(range(10)):
        ...
        for j in range(500):
            ...
            if done:
                print('end')
                break
            #if sumtree.addnum%replay_inter ==0 and sumtree.addnum/replay_inter>5:
                #print('do')
            if sumtree.addnum> replay_inter:
                ...
                for state, action, reward, next_state, done in minibatch:
                    ...
                    if done:
                        ...
                        #print('here',action_target, model_.predict(next_state))
                    
                    target = model.predict(state)
                    #print(target,'target',traget_value_action)
                    ...
                    #print(target_with_weight)
                    starttime=time.time()
                    model.fit(state,target_with_weight,epochs=2, verbose=0)
                    endtime=time.time()
                    #print(endtime-starttime)
                    idx_num = idx_num + 1
                    print('122344567',action_target,model_.predict(next_state))
